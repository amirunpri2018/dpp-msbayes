...
int makeQHgametes(int n,int st,int end,struct node *tre,int ns,struct QHparameters *QH,char **);
...
/* Aug 11, 2004 Eli changes */
	if (P.seQmut) {
	  ...
	  P.QH->MRCAseq = (char *)malloc( (unsigned)( (P.nsites)*sizeof(char)) ) ;
	  P.QH->treeseq = (char *)malloc( (unsigned)( (2*P.nsam-1)*sizeof(char)) ) ;
	  posit = (double *)malloc( (unsigned)( maxsites*sizeof( double)) ) ;
 	  if (P.QH->output) list = cmatrix(P.nsam,P.nsites+1);
	  else list = cmatrix(P.nsam,maxsites+1);
	} else {
/* end Eli */
	  if( P.theta > DBL_EPSILON ) {
	    ...
	    posit = (double *)malloc( (unsigned)( P.segsites*sizeof( double)) ) ;
	  }
	} //Aug 11, 2004 Eli changes
...
/* Aug 11, 2004 Eli changes */
	    if ( (P.seQmut) ) {
	      ...
	      fprintf(pf,"\nfreqACGT:");
	      for (i=0;i<4;i++) fprintf(pf," %6.4f",P.QH->obsfreq[i] );
...
int gensam(struct parameters P, char **list)
{
...
/* 1 Nov 01 Eli changes */
	if (P.seQmut) {
	  ns = 0 ;
	  for(i=0;i<4;i++) P.QH->obsfreq[i]=0.;
	  createrandomseQ(P.QH->MRCAseq,P.nsites,P.QH);
	  for( seg=0, k=0; k<nsegs; seg=seglst[seg].next, k++) {
	    start = seglst[seg].beg ;
	    end = ( k<nsegs-1 ? seglst[seglst[seg].next].beg -1 : P.nsites-1 );
	    assign_ndes(P.nsam,seglst[seg].ptree);
	    segsit = makeQHgametes(P.nsam,start,end,seglst[seg].ptree,ns,P.QH,list);
	    free(seglst[seg].ptree);
	    ns += segsit;
	  }
	  for(i=0;i<4;i++) P.QH->obsfreq[i] /= (P.nsam*P.nsites);
	} else {
/* end Eli */
	  if( P.theta > DBL_EPSILON ) {
	    ns = 0 ;
...
/* 	      if (seg==0) *pttot = tt; */
	    ...
	    for( seg=0, k=0; k<nsegs; seg=seglst[seg].next, k++) { 
	      ...
	      len = end - start + 1 ;
	      tseg = (double)len/(double)P.nsites;
	      make_gametes(P.nsam,seglst[seg].ptree,tt*pk[k]/tseg, ss[k], ns, list);
	      free(seglst[seg].ptree) ;
	...
	for(i=0;i<P.nsam;i++) list[i][(P.seQmut)&&(P.QH->output)?P.nsites:ns] = '\0' ;   // Aug 11 2004  Eli
	return( ns ) ;
}
...
/* Eli new functions */
  ...
  int
makeQHgametes(int n,int start,int end,struct node *ptree, int ns,struct QHparameters *QH, char **list)
{
  int ni, site, mut, segsites, i, baseint(char), segbool;
  char intbase(int);
  double muttime, tt, newttime(struct node *ptree, int n, int nodenum), ttthisbranch;
  double urandev, ran1();
  double rate, calcRate(char,struct QHparameters *QH), rategammafactor=0, gammadev(double alpha);
...
/*   length = end-start+1;  nodenum = 2*n - 1; */
  ...
  for (i=0;i<4;i++) baseRate[i] = calcRate(intbase(i),QH) * QH->siterate ;
  site =start;
  while (site <= end) {
    ...
    if (QH->alpha) rate *= (rategammafactor=gammadev(QH->alpha));
    while ( muttime > (rate*tt) ) {
      for (ni=0;ni<n;ni++) QH->treeseq[ni] = QH->treeseq[2*n-2] ;
...
/*       fprintf(stderr,"muttime %7.4f tt %6.4f",muttime, tt); */
      if ((muttime<=0.)||(muttime>=tt)) perror("mkQHgamR: mutate site initial muttime calc wrong");
      /* mutate this site */
      nlineages = 2;
      for (i=0;i<4;i++) baselineages[i]= (i==baseint(QH->treeseq[2*n-2]))? 2: 0;
...
/* 	fprintf(stderr,"\nabv node time: %10.8f, node time: %10.8f, %1d lineages",(ptree+ni+1)->time,(ptree+ni)->time,nlineages); */
	ttthisbranch = ( ((ptree+ni+1)->time - (ptree+ni)->time) * nlineages ) ;
/* 	fprintf(stderr,"\nsite %1d, %1d lineages, ttthisbranch: %10.8f",site, nlineages,ttthisbranch); */
	while ( (ttthisbranch -= muttime) > 0 ) {
	  ...
	  for (i=0,lineageratesum=0.;i<4;i++) lineageratesum += baseRate[i] * baselineages[i];
	  while ((urandev=ran1())== 1.) ;
	  for (randomlineage=ni,lineageprob=0.;randomlineage>=0;randomlineage--) {
	    if ( (ptree+((ptree+randomlineage)->abv))->time > (ptree+ni)->time )
	      if (urandev<(lineageprob+=(baseRate[baseint(QH->treeseq[randomlineage])]/lineageratesum)))
		{ mut=1; break; }
	  }
	  if (randomlineage<0) perror("\nmakeQHgametesR: mutate site pick randomlineage error");
	  baselineages[baseint(QH->treeseq[randomlineage])]--;
	  QH->treeseq[randomlineage] = mutateRate(QH->treeseq[randomlineage],QH);
	  mutateBelow(n,ptree,randomlineage,QH->treeseq);
	  baselineages[baseint(QH->treeseq[randomlineage])]++;
	  /* recalculate muttime(nlineages,baselineages,baseRates) */
	  for(i=0,rate=0.;i<4;i++) rate += baseRate[i] * baselineages[i] ;
	  if (QH->alpha) rate *= rategammafactor;
	  rate /= nlineages;
	  while ((urandev=ran1())== 0.) ;
	  muttime = - log(urandev) / rate;
	}
	if (fabs((ptree+ni)->time) <1e-6) break;
	if ((ptree+ni)->ndes > 1) {
	  nlineages++;
	...
	if (QH->output) list[i][site]=QH->treeseq[i];
      }
      for (i=1,segbool=0;i<n;i++) if (QH->treeseq[i]!=QH->treeseq[0])
	{ segbool=1; break; }
/*   fprintf(stderr,"\nHERE "); */
/*   fprintf(stderr,"HERE2\n"); */
      if (segbool) {
...
/* 	  printf("\nREALLOC "); */
	  ...
	  posit = (double *)realloc(posit, maxsites*sizeof(double) ) ;
	  if (!(QH->output)) biggerlist(n,list);
/* 	  printf("done\n"); */
	}
/* 	printf("- updating-"); */
	posit[ns+segsites] = site;
	if (!QH->output) for (i=0;i<n;i++) list[i][ns+segsites]=QH->treeseq[i];
	segsites++;
/* 	printf("done "); */
      }
      site++;
...
{
  int i,ii,done;
  for (i=0;i<(2*n-1);i++) (tree+i)->ndes = (i<n)? 1: 0;
  for (i=n;i<(2*n-1);i++) 
     ...
     int nsam, nodenum;
{
  ...
  for (i=(nodenum-2);i>=0;i--) {
    ...
    if ((ptree+i)->ndes >1) nlineages++;
    else if ((ptree+i)->ndes ==1) nlineages--;
    if (fabs((ptree+i)->time) <1e-6) break;
  }
  return(t);
}
