...
/* Initialization */
	...
	if( config == NULL ) perror("malloc error. segtre. config");
	for(pop=0;pop<npop;pop++) config[pop] = P.config[pop] ;
	...
	if( alphag == NULL ) perror("malloc error. segtre. alphag");
	eventtimes = (double *)malloc( (unsigned)(npop*npop+1)*sizeof(double) ) ;
	if( eventtimes == NULL ) perror("malloc error. segtre. eventtimes");
	if (tpast>DBL_EPSILON)
	  for (pop=0;pop<npop;pop++)
	    if ( fabs((P.Dint)->Npast[pop] - (P.Dint)->Nrecent[pop]) < DBL_EPSILON ) alphag[pop] = 0.;
	    else alphag[pop] = -log( (P.Dint)->Npast[pop]/(P.Dint)->Nrecent[pop] ) / ( tpast-prevtpast ) ; 
	else {
	  if (P.D) Dint_updateconfig(P, ++Dint, &config );
	  tpast = (P.Dint+Dint)->Dtpast ;
	  npop = (P.Dint+Dint)->npops ;
	  alphag = (double *)realloc( alphag, (unsigned)(npop*sizeof(double))) ;
	  if (alphag==NULL) perror("realloc error. segtre init alphag[pop]\n");
	  eventtimes = (double *)realloc( eventtimes, (unsigned)(npop*npop+1)*sizeof(double)) ;
	  if (eventtimes==NULL) perror("realloc error. segtre init eventtimes\n");
	  for (pop=0;pop<npop;pop++)
	    if ( (tpast - prevtpast < DBL_EPSILON)||(fabs((P.Dint+Dint)->Npast[0]-(P.Dint+Dint)->Nrecent[0])<DBL_EPSILON) ) alphag[pop] = 0.;
	    else alphag[pop] = -log( (P.Dint+Dint)->Npast[pop]/(P.Dint+Dint)->Nrecent[pop] ) / ( tpast-prevtpast ) ;
	...
	nsegs=1;
	r /= (nsites-1);
	if( f > DBL_EPSILON ) pc = (P.track_len -1.0)/P.track_len ;
	else pc = 1.0 ;
	lnpc = log( pc ) ;
	cleft = nsam* ( 1.0 - pow( pc, (double)(nsites-1) ) ) ;
	rft = f * P.track_len ;
	if (r>DBL_EPSILON) rft *=r;
	...
	while( nchrom > 1 ) {
	    ...
	    } else eventtimes[event++] = -1.0 ; /* no recombination within interval */
	    ...
	    for(pop=0; pop<npop; pop++)
		  ...
		  if (fabs(alphag[pop])<DBL_EPSILON) {
		    eventtimes[event++] = -log( rdum )*(P.Dint+Dint)->Nrecent[pop] / (config[pop]*(config[pop]-1)) ;
		  } else {
		    arg=1.-log(rdum)*(P.Dint+Dint)->Nrecent[pop]*exp(-alphag[pop]*(t-prevtpast))*alphag[pop]/(config[pop]*(config[pop]-1));
		    if( arg > 0. ) eventtimes[event++] = log( arg ) / alphag[pop] ;
		    else eventtimes[event++] = -1. ; /* no coalescent within interval */
		  }
		} else eventtimes[event++] = -1. ;
	    if (event!=1+npop) {
	      fprintf(stderr,"error- segtre- coal eventtimes[] assignment, event=%1d, (1+npop)=%1d\n",event,(1+npop));exit(1);}
	    /* migration times */
	    if (npop>1)
	      for(pop=0; pop<npop; pop++)
	        if (config[pop]>0) {
			...
			if (pop2 != pop) 
			  if (M[pop][pop2]>DBL_EPSILON) {
				...
				eventtimes[event++] = -log(rdum)/(config[pop]*M[pop][pop2]) ;
			      else if ( Mpattern == 1 ) /* migration dependent on pop but not source pop */
				if (fabs(alphag[pop])<DBL_EPSILON)
				  eventtimes[event++] = -log(rdum)*((P.Dint+Dint)->Nrecent[pop]*exp(-alphag[pop]*(t-prevtpast))) /
				    (config[pop]*M[pop][pop2]);
				else {
				  arg=1.-log(rdum)*((P.Dint+Dint)->Nrecent[pop]*exp(-alphag[pop]*(t-prevtpast)))*alphag[pop] /
				    (config[pop]*M[pop][pop2]);
				  if (arg>0.) eventtimes[event++] = log( arg ) / alphag[pop] ;
				  else eventtimes[event++] = -2. ; /* no migration[pop][pop2] within interval */
				}
			      else /* if ( Mpattern == 2 ) *//* migration dependent on pop and source pop */
				if (fabs(alphag[pop]-alphag[pop2])<DBL_EPSILON)
				  eventtimes[event++] = -log(rdum)*((P.Dint+Dint)->Nrecent[pop]*exp(-alphag[pop]*(t-prevtpast))) /
				    (config[pop]*M[pop][pop2]*((P.Dint+Dint)->Nrecent[pop2]*exp(-alphag[pop2]*(t-prevtpast))));
				else {
				  arg=1.-log(rdum)*((P.Dint+Dint)->Nrecent[pop]*exp(-alphag[pop]*(t-prevtpast)))*(alphag[pop]-alphag[pop2]) /
				    (config[pop]*M[pop][pop2]*((P.Dint+Dint)->Nrecent[pop2]*exp(-alphag[pop2]*(t-prevtpast))));
				  if (arg>0.) eventtimes[event++] = log(arg) / (alphag[pop]-alphag[pop2]) ;
				  else eventtimes[event++] = -2. ; /* no migration[pop][pop2] within interval */
				}
			  } else eventtimes[event++] = -2. ; /* else- no migration within interval */
		} else
		  for (pop2=0;pop2<npop;pop2++)
		    if (pop2 != pop) eventtimes[event++] = -2. ; /* else- no migration within interval */
	    if (event!=1+npop*npop) {
	      fprintf(stderr,"error- segtre- mig eventtimes[] assignment, event=%1d, (1+npop*npop)=%1d\n",event,(1+npop*npop));exit(1);}
	    ...
	    for (i++;i<(1+npop*npop);i++)
	      if ((eventtimes[i]>DBL_EPSILON)&&(eventtimes[i]<eventtimes[event]))
		event = i;
	    if ( event == (1+npop*npop) ) t = tpast + 1. ; // no event in interval
	    else t += eventtimes[event] ;
		...
		tpast = (P.Dint+Dint)->Dtpast ;
		alphag = (double *)realloc( alphag, (unsigned)(npop*sizeof(double))) ;
		if (alphag==NULL) perror("realloc error. segtre alphag[pop]\n");
		eventtimes = (double *)realloc( eventtimes, (unsigned)(npop*npop+1)*sizeof(double)) ;
		if (eventtimes==NULL) perror("realloc error. segtre init eventtimes\n");
		for (pop=0;pop<npop;pop++)
		  if ( (tpast - prevtpast < DBL_EPSILON)||(fabs((P.Dint+Dint)->Npast[0]-(P.Dint+Dint)->Nrecent[0])<DBL_EPSILON) ) alphag[pop] = 0.;
...
/* 		    printf("\n"); */
	        ...
	        if( event-- ==0 ) {			     /* recombination event ? */
		    if( (ran = ran1()) < ( prec / prect ) ){   /* Xover */
		        rchrom = re(nsam);
		...
		} else if ( event < npop )  { 		     /* coalescent event */
...
/* 	      fprintf(stderr," \n got here2 migrant=%1d nchrom=%1d migchrom=%1d", migrant, nchrom, migchrom); */
		    if (migchrom>=nchrom) perror("error- segtre- migchrom assignment\n");
...
/* Old Main loop */
	...
	while( nchrom > 1 ) {
	    ...
	    if( coal_prob > 0.0 ) {
		while( ( rdum = ran1() )  == 1.0 ) ;
		if( oldalphag == 0 ) tcoal = -log( 1.-rdum )*(P.Dint+Dint)->Nrecent[0] /coal_prob ;
		else {
		  arg  = 1. - oldalphag*(P.Dint+Dint)->Nrecent[0]*exp(-oldalphag*(t-prevtpast) )* log(1.-rdum) / coal_prob     ;
		  if( arg <= 0.0 )  tcoal = tpast + 1.0 ; /* no coalescent within interval */
		  else tcoal = log( arg ) / oldalphag ;
		    }
	        }

	   if( tcoal >= 0.0 ) {
		if( trm >= 0.0 ) ttemp = MIN( trm, tcoal) ;
		else{
			ttemp = tcoal;
			trm = tcoal + 1.0 ;
		  	}
	   ...
	   //	      printf("\ndint: %1d nchrom: %1d tcoal %9.7f alphag %9.7f\n", Dint, nchrom, tcoal, oldalphag);
		...
		if( (Dint < lastDint ) && ( t + ttemp > tpast ) ) {
		  /* UPDATE */
			...
			++Dint ;
			tpast = (P.Dint+Dint)->Dtpast ;
			if( tpast - prevtpast < DBL_EPSILON ) oldalphag = 0.0 ;
			else  oldalphag = -log( (P.Dint+Dint)->Npast[0]/(P.Dint+Dint)->Nrecent[0] ) / ( tpast-prevtpast ) ; 
		        npop = (P.Dint+Dint)->npops ;
...
/* 	      printf("\nevent-rec dint: %1d prevtpast %9.7f t %9.7f tpast %9.7f\n", Dint, prevtpast, t, tpast); */
		   ...
		   else if( ran < (prec + clefta)/(prect+mig) ){    /*  cleft event */
			rchrom = cleftr(nsam);
			 config[ chrom[rchrom].pop ] += 1 ;
			}
		   else if( ran < prect/(prect+mig) ) {         /* cin event */
			rchrom = cinr(nsam,nsites);
			if( rchrom >= 0 ) config[ chrom[rchrom].pop ] += 1 ;
			}
		   else  {  /* migration event */
...
/* 		    for (i=0;i<npop;i++) fprintf(stderr,"%1d,", config[i]); */
			...
			prob = 0.0 ;
			for(pop=0;pop<npop;pop++) {
				prob += ((double)config[pop])*(config[pop]-1.)/coal_prob ;
				if( ran < prob ) break;
				}
...
Dint_updateconfig(struct parameters P, int Dint, int **pconfig)
{
	...
	oldconfig = (int *)malloc( (unsigned)(P.Dint+Dint-1)->npops*sizeof(int) ) ;
