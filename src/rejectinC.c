...
If simulated statistics are equal to 'nan' or 'inf', a warning message is written to standard error (should
 ...
 component of Euclidean distance for that statistic equal to 3 (i.e. 3 sd's from the mean, or "kinda far")
 is arbitrarily substituted. 
...
usage(int exitstatus)
{
  ...
  printf("\nrejectinC, or rejeCt, does the acceptance/rejection step of Hickerson et al's ABC algorithm in C\n");
  printf("  (rather than R, which uses A LOT of memory. This program expects a file of observed summary statistics\n");
  printf("  and a file of simulated summary statistics, in the SAME FORMAT (same column contains same summary\n");
  printf("  statistic, e.g. Tajima's D, in both files); input filenames are the first two commandline arguments. \n");
  printf("  The third argument is the tolerance, or the proportion of simulation replicates that will be accepted. \n");
  printf("  The fourth and following arguments are the columns in the summary statistic files to be considered. \n");
  printf("\nrejeCt opens the observed stats file and reads the appropriate columns into memory. Then it opens the \n");
  printf("  simulated stats file and reads through it calculating a mean and sd for each of the appropriate columns.\n");
  printf("  It then normalizes the observed stats (in memory). It then reopens the sim stats file and reads in the\n");
  printf("  appropriate stats, and normalizes them by mean and sd and calculates the Euclidian distance between that\n");
  printf("  simulation replicate and the observed data. This program keeps the proportion tolerance (the number equal\n");
  printf("  to tolerance times the number of simulation replicates, i.e. lines in the sim stats file) of the \n");
  printf("  simulation replicates with the lowest Euclidean distance from the observed. For these simulation\n");
  printf("  replicates, the entire lines from the sim stats file are written to standard out. \n");
  printf("\nIf simulated statistics are equal to 'nan' or 'inf', a warning message is written to standard error (should\n");
  printf("  show up on the terminal). These statistics are not considered when calculating means and sd's, and the\n");
  printf("  component of Euclidean distance for that statistic equal to 2 (i.e. 2 sd's from the mean, or \"kinda far\")\n");
